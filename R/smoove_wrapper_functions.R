...
#'
#' This function estimates the mean speed \eqn{nu}, the time-scale \eqn{tau} and (occasionally) the
#' initial speed \eqn{v_0} of the unbiased correlated velocity movement (UCVM).
#' See Gurarie et al. (2016) and the \code{vignette("smoove", package = "smoove")} vignette for
#' more details.
#'
#' @param data an object coercible to class \code{trackframe}
#' @param method the method to use for the estimation.  These are (in increasing:
#' velocity auto-correlation fitting (\code{vaf}), correlated random walk matching (\code{crw}),
#' velocity likelihood (\code{vLike}), position likelihood (\code{zLike}) and position likelihood
#' with Kalman filter (\code{crawl}). This last method is generally he best method, since it  fits
#' the position likelihood more efficiently by using a Kalman filter. It is based on
#' Johnson et al (2008) and is a wrapper for the \code{\link[crawl]{crwMLE}} in the (excellent)
#' \code{\link[crawl]{crawl}} package. The default method is \code{vLike}.
#' @param parameters which parameters to estimate.  For most methods "tau" and "nu" are always both
#' estimated, but some computation can be saved for the velocity likelihood method by providing
#' an estimate for "nu".
#' @param ci whether or not to compute 95\% confidence intervals for parameters. In some cases,
#' this can slow the computation down somewhat.
#' @param spline whether or not to use the spline correction (only relevant for \code{vaf} and
#' \code{vLike}).
#' @param diagnose whether to draw a diagnostic plot.  Varies for different methods.
#' @param time_units unit of time to be passed to \code{\link[base]{difftime}()} to compute a
#' relative numeric time vector.
#' @param ... additional parameters to pass to estimation functions.  These are particularly
#' involved in the \code{crawl} method (see \code{\link[crawl]{crwMLE}}).
...
#'
#' This group of functions estimate the parameters of a rotational and advective CVM using a
#' one-step velocity likelihood.  It is best implemented on relatively high resolution data from
#' which one can obtain good estimates of velocity.  The observations can, however, be irregularly
#' sampled.
#'
#' The parameterization is: \eqn{\tau} - characteristic time scale, \eqn{\mu} - advective velocity,
...
#' @param compare_models whether to compare four models: with both rotation and advection, only
#' rotation, only advection, or neither.  The comparison provides a table with the log likelihood,
#' number of parameters, AIC, BIC, delta AIC and delta BIC values.  A limited comparison set may be
#' useful when running the fit many times (e.g. when performing change point analysis).
#' @param modelset which models to fit and compare (if \code{compare_models}) is TRUE)
#' @param p0 optional named list of initial parameter values in the form: c(tau, eta, omega, mu.x,
#' mu.y).
#' @param spline whether to implement the spline correction on the positions
#' @param spline_res resolution of spline (see \code{\link[smoove]{getV.spline}})
#' @param t_spline new times for spline estimation (best left as NULL)
#' @param time_units time units of calculations (e.g. "secs", "mins", "hours", "days")
...
#' Sweep RACVM
#'
#' Sets a window (a subset of movement data within specific time window), computes likelihoods for
#' a set of candidate change points within the window, and steps the window forward, filling out a
#' likelihood matrix.
#'
#' @param data an object of class \code{trackframe}
...
#' "secs", "mins", "hours", "days", "weeks" (See \code{\link[base]{difftime}}). Ignored if time is
#' not POSIX.
#' @param progress whether or not to show a progress bar
#' @param ... additional parameters to pass to the \code{\link{estimate_racvm}} function, notably
#' the option "criterion" allows you to select models based on AIC or BIC (the former is more
#' liberal with more complex models).
...
#' @param time_units time units of calculations (e.g. "secs", "mins", "hours", "days")
#' @param ... additional parameters to pass to \code{\link{estimateUCVM}} function, in particular
#' the method of estimation.  Under most conditions, fairly reliable and fast results are provided
...
#' Plot Phases
#'
#' @param x an object of class trackframe containing a cvm phases object in the attributes or a
#' phase list
...
#' @param layout  "horizontal" (default) or "vertical"- as preferred (partial string matching
#' accepted)
#'
#' @export
